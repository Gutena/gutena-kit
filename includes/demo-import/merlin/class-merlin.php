	...
	class Merlin {
		...
		function __construct( $config = array(), $strings = array() ) {
			...
			if ( ! class_exists( '\ProteusThemes\WPContentImporter2\WPImporterLogger' ) ) {
				require_once trailingslashit( $this->base_path ) . $this->directory . '/vendor/wp-content-importer-v2/WPImporterLogger.php';
			}
		...
		function required_classes() {
			...
			if ( ! class_exists( '\ProteusThemes\WPContentImporter2\WXRImporter' ) ) {
				require_once trailingslashit( $this->base_path ) . $this->directory . '/vendor/wp-content-importer-v2/WXRImporter.php';
			}

			if ( ! class_exists( '\ProteusThemes\WPContentImporter2\WXRImportInfo' ) ) {
				require_once trailingslashit( $this->base_path ) . $this->directory . '/vendor/wp-content-importer-v2/WXRImportInfo.php';
			}
			
			if ( ! class_exists( '\ProteusThemes\WPContentImporter2\Importer' ) ) {
				require_once trailingslashit( $this->base_path ) . $this->directory . '/vendor/wp-content-importer-v2/Importer.php';
			}
		...
		public function admin_page() {
			...
			if ( empty( $_GET['page'] ) || $this->merlin_url !== sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) {
				return;
			}
			...
			$texts = array(
				'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'gutena-kit' ),
			);
		...
		public function svg( $args = array() ) {
			...
			if ( empty( $args ) ) {
				return __( 'Please define default parameters in the form of an array.', 'gutena-kit' );
			}

			// Define an icon.
			if ( false === array_key_exists( 'icon', $args ) ) {
				return __( 'Please define an SVG icon filename.', 'gutena-kit' );
			}
		...
		<?php
			$this->logger->debug( __( 'The welcome step has been displayed', 'gutena-kit' ) );
		}
		...
		protected function license() {
			...
			<?php
			$this->logger->debug( __( 'The license activation step has been displayed', 'gutena-kit' ) );
		}
		...
		<?php
			$this->logger->debug( __( 'The child theme installation step has been displayed', 'gutena-kit' ) );
		}
		...
		protected function plugins() {
			...
			$method = '';
			$fields = array_map( 'sanitize_text_field', array_keys( $_POST ) );
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
										<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'gutena-kit' ); ?>">
											<?php esc_html_e( 'req', 'gutena-kit' ); ?>
										</span>
									</span>
								</label>
							</li>
		...
		<?php
			$this->logger->debug( __( 'The plugin installation step has been displayed', 'gutena-kit' ) );
		}
		...
		<?php
			$this->logger->debug( __( 'The final step has been displayed', 'gutena-kit' ) );
		}
		...
		public function generate_child() {
			...
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
				$this->logger->error( __( 'Failed to generate the child theme, because of incorrect data.', 'gutena-kit' ) );

				wp_send_json_error(
					array(
						'error'   => 1,
						'message' => esc_html__( 'Invalid data!', 'gutena-kit' ),
					)
				);
			}
			...
			if ( ! file_exists( $path ) ) {
				...
				$this->logger->debug( __( 'The existing child theme was activated', 'gutena-kit' ) );
			...
			$this->logger->debug( __( 'The newly generated child theme was activated', 'gutena-kit' ) );
		...
		public function _ajax_activate_license() {
			...
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
				wp_send_json(
					array(
						'success' => false,
						'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'gutena-kit' ),
					)
				);
			}
			...
			if ( empty( $_POST['license_key'] ) ) {
				wp_send_json(
					array(
						'success' => false,
						'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'gutena-kit' ),
					)
				);
			}
			...
			$this->logger->debug( __( 'The license activation was performed with the following results', 'gutena-kit' ), $result );
		...
		protected function edd_activate_license( $license ) {
			...
			if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$message = esc_html__( 'An error occurred, please try again.', 'gutena-kit' );
				}
			} else {
					...
					switch ( $license_data->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license key expired on %s.', 'gutena-kit' ),
								date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
							);
							break;

						case 'revoked':
							$message = esc_html__( 'Your license key has been disabled.', 'gutena-kit' );
							break;

						case 'missing':
							$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'gutena-kit' );
							break;

						case 'invalid':
						case 'site_inactive':
							$message = esc_html__( 'Your license is not active for this URL.', 'gutena-kit' );
							break;

						case 'item_name_mismatch':
							/* translators: EDD Item Name */
							$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'gutena-kit' ), $this->edd_item_name );
							break;

						case 'no_activations_left':
							$message = esc_html__( 'Your license key has reached its activation limit.', 'gutena-kit' );
							break;
		...
		public function generate_child_functions_php( $slug ) {
			...
			$this->logger->debug( __( 'The child theme functions.php content was generated', 'gutena-kit' ) );
		...
		public function generate_child_style_css( $slug, $parent, $author, $version ) {
			...
			$this->logger->debug( __( 'The child theme style.css content was generated', 'gutena-kit' ) );
		...
		public function generate_child_screenshot( $path ) {
			...
			if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
				...
				$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'gutena-kit' ), array( 'copied' => $copied ) );
			} else {
				$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'gutena-kit' ), array( 'screenshot' => $screenshot ) );
			}
		}
		...
		function _ajax_plugins() {
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating', 'gutena-kit' ),
					);
					break;
				}
			}
			...
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating', 'gutena-kit' ),
					);
					break;
				}
			}
			...
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] === $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing', 'gutena-kit' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				$this->logger->debug(
					__( 'A plugin with the following data will be processed', 'gutena-kit' ),
					array(
				...
				$json['message'] = esc_html__( 'Installing', 'gutena-kit' );
				wp_send_json( $json );
			} else {
				$this->logger->debug(
					__( 'A plugin with the following data was processed', 'gutena-kit' ),
					array(
						'plugin_slug' => sanitize_key( wp_unslash( $_POST['slug'] ) ),
					)
				);
				...
				wp_send_json(
					array(
						'done'    => 1,
						'message' => esc_html__( 'Success', 'gutena-kit' ),
					)
				);
			}
		...
		function _ajax_content() {
			...
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'gutena-kit' ) );

				wp_send_json_error(
					array(
						'error'   => 1,
						'message' => esc_html__( 'Invalid content!', 'gutena-kit' ),
					)
				);
			}
			...
			if ( isset( $_POST['proceed'] ) ) {
				if ( is_callable( $this_content['install_callback'] ) ) {
					$this->logger->info(
						__( 'The content import AJAX call will be executed with this import data', 'gutena-kit' ),
						array(
					...
					if ( $logs ) {
						...
						if ( 'content' === sanitize_text_field( wp_unslash( $_POST['content'] ) ) ) {
			...
			if ( $json ) {
				...
				$this->logger->error(
					__( 'The content import AJAX call failed with this passed data', 'gutena-kit' ),
					array(
				...
				wp_send_json(
					array(
						'error'   => 1,
						'message' => esc_html__( 'Error', 'gutena-kit' ),
						'logs'    => '',
						'errors'  => '',
					)
				);
		...
		public function _ajax_get_total_content_import_items() {
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
				$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'gutena-kit' ) );
		...
		protected function get_import_data( $selected_import_index = 0 ) {
			...
			if ( ! empty( $import_files['content'] ) ) {
				$content['content'] = array(
					'title'            => esc_html__( 'Content', 'gutena-kit' ),
					'description'      => esc_html__( 'Demo content data.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
			...
			if ( ! empty( $import_files['widgets'] ) ) {
				$content['widgets'] = array(
					'title'            => esc_html__( 'Widgets', 'gutena-kit' ),
					'description'      => esc_html__( 'Sample widgets data.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
			...
			if ( ! empty( $import_files['sliders'] ) ) {
				$content['sliders'] = array(
					'title'            => esc_html__( 'Revolution Slider', 'gutena-kit' ),
					'description'      => esc_html__( 'Sample Revolution sliders data.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
			...
			if ( ! empty( $import_files['options'] ) ) {
				$content['options'] = array(
					'title'            => esc_html__( 'Options', 'gutena-kit' ),
					'description'      => esc_html__( 'Sample theme options data.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
			...
			if ( ! empty( $import_files['redux'] ) ) {
				$content['redux'] = array(
					'title'            => esc_html__( 'Redux Options', 'gutena-kit' ),
					'description'      => esc_html__( 'Redux framework options.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
			...
			if ( false !== has_action( 'merlin_after_all_import' ) ) {
				$content['after_import'] = array(
					'title'            => esc_html__( 'After import setup', 'gutena-kit' ),
					'description'      => esc_html__( 'After import setup.', 'gutena-kit' ),
					'pending'          => esc_html__( 'Pending', 'gutena-kit' ),
					'installing'       => esc_html__( 'Installing', 'gutena-kit' ),
					'success'          => esc_html__( 'Success', 'gutena-kit' ),
		...
		public function import_revolution_sliders( $file ) {
			...
			$this->logger->info( __( 'The revolution slider import was executed', 'gutena-kit' ) );
		...
		public function pt_importer_new_ajax_request_response_data( $data ) {
			...
			$data['message']  = esc_html__( 'Installing', 'gutena-kit' );
			$data['proceed']  = 'true';
		...
		public function after_content_import_setup() {
			...
			if ( $homepage ) {
				...
				$this->logger->debug( __( 'The home page was set', 'gutena-kit' ), array( 'homepage_id' => $homepage ) );
			}
			...
			if ( $blogpage ) {
				...
				$this->logger->debug( __( 'The blog page was set', 'gutena-kit' ), array( 'blog_page_id' => $blogpage ) );
			}
		}
		...
		public function before_content_import_setup() {
			...
			if ( ! empty( $hello_world ) ) {
				...
				$this->logger->debug( __( 'The Hello world post status was set to draft', 'gutena-kit' ) );
			}
		}
		...
		public function validate_import_file_info( $import_files ) {
			...
			foreach ( $import_files as $import_file ) {
				if ( ! empty( $import_file['import_file_name'] ) ) {
					...
					$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'gutena-kit' ), $import_file );
				}
			}
		...
		public function update_selected_import_data_info() {
			...
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
				$this->logger->error( __( 'The content importer update info failed, because of incorrect data.', 'gutena-kit' ) );
		...
		public function import_finished() {
			...
			if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
				$this->logger->error( __( 'The content importer finish failed, because of incorrect data.', 'gutena-kit' ) );
